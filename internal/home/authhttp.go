...
func (a *Auth) newCookie(req loginJSON, addr string) (c *http.Cookie, err error) {
	rateLimiter := a.rateLimiter
	u, ok := a.findUserByName(req.Name)
	if !ok {
		if rateLimiter != nil {
			rateLimiter.inc(addr)
		}
		return nil, errors.Error("invalid username")
	}
...
func handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	// Verify PASETO token
	valid, err := VarifyTokenPaseto(req.Token)
	if err != nil {
		writeErrorWithIP(r, w, http.StatusForbidden, remoteIP, "auth: invalid token: %s", err)
		return
	}
	if !valid {
		writeErrorWithIP(r, w, http.StatusForbidden, remoteIP, "auth: invalid token")
		return
	}
